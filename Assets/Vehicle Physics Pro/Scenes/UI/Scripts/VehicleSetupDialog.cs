...
namespace VehiclePhysics.UI
{
    public class VehiclePhysicsUI : MonoBehaviour
    {
        public VehicleBase vehicle; // Reference to the vehicle

        // Sets a vehicle setting based on the provided setting ID and value
        public void SetVehicleSetting(int setting, int value)
        {
            if (vehicle != null)
                vehicle.data.Set(Channel.Settings, setting, value);
        }

        // Sets a vehicle input based on the provided input ID and value
        public void SetVehicleInput(int input, int value)
        {
            if (vehicle != null)
                vehicle.data.Set(Channel.Input, input, value);
        }
        ...
        public bool IsEnabled(Toggle toggle)
        {
            return toggle != null ? toggle.isOn : false;
        }

        // Retrieves the VPVehicleController component from the vehicle
        public VPVehicleController GetVPVehicleController()
        {
            if (vehicle == null) return null;
            return vehicle.GetComponent<VPVehicleController>();
        }
